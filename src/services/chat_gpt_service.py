...
async def get_chat_gpt_response(message: discord.Message, bot: discord.Client):
    ...
    word_limit = random.randint(200, 300)
    ...
    # Force web search when the user explicitly requests it.
    # Otherwise, allow the model to decide.
    tool_choice = (
        "required"
        if explicitly_requests_search(text_prompt)
        else "auto"
    )
    ...
    if has_image:
        ...
        response = await client.responses.create(
            ...
            instructions=(
                ...
                For current, live, latest, recent, patch, version, release, schedule,
                news, or status questions, search the web before answering.

                Never provide a preliminary answer before searching.
                Never include an outdated guess followed by a correction.
                Never claim that you cannot search the web.
                After searching, provide exactly one final answer based on the search results.
            """
            ),
            tools=[
                {
        ...
        answer = get_final_response_text(response)

        return re.sub(
            r"(?i)l\s*a\s*d\s*d\s*e\s*r",
            "climbing device",
            answer[:2000],
        )
...
def get_final_response_text(response) -> str:
    """
    Return only the final output_text block generated by the assistant.
    """
...
def explicitly_requests_search(prompt: str) -> bool:
    return bool(
        re.search(
            r"\b("
            r"please\s+search|"
            r"search\s+(?:the\s+)?web|"
            r"look\s+(?:it|this|that)\s+up|"
